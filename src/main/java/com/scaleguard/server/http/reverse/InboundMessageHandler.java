...
public class InboundMessageHandler {
  ...
  public CachedResponse getKeyData(TargetSystem ts,Object msg){
    ...
    if (msg instanceof HttpRequest) {
      ...
      List<CachedResource> crs= ts.getCachedResources().stream().filter(crx->crx.getMethod().equalsIgnoreCase(request.method().name()) && request.uri().split("[?]")[0].startsWith(crx.getPattern())).collect(Collectors.toList());
      if(crs.size()>0) {
        ...
        if(cachedResource.isAsync()){
          ...
          pr.setUri(request.getUri());
          pr.setMethod(request.getMethod().name());
  ...
  private String toKey(CachedResource crs,HttpRequest request){
    ...
    onode.put("method",request.getMethod().name());
    onode.put("uri",request.getUri());
